...
#include "mc_power_move_absolute_lib.hpp"
...
LocatedVar locatedVars[13] = {
    ...
    { LocatedArea::Input, LocatedSize::LWord, 8, 0, {0, 0, 0}, nullptr },  // TARGETPOS AT %IL8
    { LocatedArea::Input, LocatedSize::LWord, 16, 0, {0, 0, 0}, nullptr },  // VELOCITY AT %IL16
    { LocatedArea::Input, LocatedSize::LWord, 24, 0, {0, 0, 0}, nullptr },  // ACCEL AT %IL24
    { LocatedArea::Input, LocatedSize::LWord, 32, 0, {0, 0, 0}, nullptr },  // DECEL AT %IL32
...
Program_MC_POWER_MOVE_ABSOLUTE_LIB::Program_MC_POWER_MOVE_ABSOLUTE_LIB()
    : ENABLECMD(false), EXECUTECMD(false), TARGETPOS(0.0), VELOCITY(0.0), ACCEL(0.0), DECEL(0.0), POWERSTATUS(false), POWERVALID(false), MOVEBUSY(false), MOVEDONE(false), MOVEERROR(false), MOVEERRORID(0), ACTUALPOS(0.0)
{
    // Initialize located variable pointers
    locatedVars[0].pointer = ENABLECMD.raw_ptr();
    locatedVars[1].pointer = EXECUTECMD.raw_ptr();
    locatedVars[2].pointer = TARGETPOS.raw_ptr();
    locatedVars[3].pointer = VELOCITY.raw_ptr();
    locatedVars[4].pointer = ACCEL.raw_ptr();
    locatedVars[5].pointer = DECEL.raw_ptr();
    ...
    locatedVars[11].pointer = MOVEERRORID.raw_ptr();
    locatedVars[12].pointer = ACTUALPOS.raw_ptr();
}

void Program_MC_POWER_MOVE_ABSOLUTE_LIB::run() {
    AXIS.AXISINDEX = 0;
    POWER.AXIS = AXIS;
    POWER.ENABLE = ENABLECMD;
    POWER();
    MOVEABS.AXIS = AXIS;
    MOVEABS.EXECUTE = EXECUTECMD;
    MOVEABS.POSITION = TARGETPOS;
    MOVEABS.VELOCITY = VELOCITY;
    MOVEABS.ACCELERATION = ACCEL;
    MOVEABS.DECELERATION = DECEL;
    MOVEABS();
    READPOS.AXIS = AXIS;
    READPOS.ENABLE = true;
    READPOS();
